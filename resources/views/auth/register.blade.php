<x-guest-layout>
    <x-authentication-card>
        ...
        @if($invite = App\Models\TeamInvitation::where('token', request()->input('token'))->where('status', 0)->first())
        <form method="POST" action="{{ route('register') }}">
            ...
            <input name="invite" hidden value="{{$invite->id}}" />
        ...
        <form method="GET" action="/">
            <div class="mt-4">
                <x-label for="name" value="{{ __('Convite inválido') }}" />
            </div>
            <div class="mt-4">
                <x-button>
                    {{ __('Voltar') }}
